...
export const createLayer_GEOSERVER = async(requestData) => {
  try {
    let res;
    if (requestData.featureTypeRequest) {
      const postData = {
        version: '2.0.0',
...
* @returns {export data}
*/
export const export_GEOSERVER = async(data) => {
  switch(data.type) {
    case 'shapezip':
...
export const export_CSV = async(exportData) => {
  try {
    const nameData = exportData.layer.groupname.split(':');
    const typeNames = exportData.layer.groupname;
    const fileName = nameData[1];
    const path = `${exportData.layer.url}/${nameData[0]}/ows?service=WFS&version=2.0.0&request=GetFeature&typeName=${typeNames}&outputFormat=csv`;
...
export const export_JSON = async(exportData) => {
  try {
    ...
    const typeNames = exportData.layer.groupname;
    const fileName = nameData[1];
    const path = `${exportData.layer.url}/${nameData[0]}/ows?service=WFS&version=2.0.0&request=GetFeature&typeName=${typeNames}&outputFormat=json`;
...
export const export_KML = async(exportData) => {
  try {
    ...
    const typeNames = exportData.layer.groupname;
    const fileName = nameData[1];
    const path = `${exportData.layer.url}/${nameData[0]}/ows?service=WFS&version=2.0.0&request=GetFeature&typeName=${typeNames}&outputFormat=kml`;
...
export const export_EXCEL = async(exportData) => {
  try {
    ...
    const typeNames = exportData.layer.groupname;
    const fileName = nameData[1];
    const path = `${exportData.layer.url}/${nameData[0]}/ows?service=WFS&version=2.0.0&request=GetFeature&typeName=${typeNames}&outputFormat=excel2007`;
...
export const export_SHAPEZIP = async(exportData) => {
  try {
    ...
    const typeNames = exportData.layer.groupname;
    const fileName = nameData[1];
    const path = `${exportData.layer.url}/${nameData[0]}/ows?service=WFS&version=2.0.0&request=GetFeature&typeName=${typeNames}&outputFormat=shape-zip&format_options=CHARSET:UTF-8`;
...
export const export_gml3 = async (exportData) => {
  try {
    ...
    const typeNames = exportData.layer.groupname;
    const fileName = nameData[1];
    const path = `${exportData.layer.url}/${nameData[0]}/ows?service=WFS&version=2.0.0&request=GetFeature&typeName=${typeNames}&outputFormat=gml3&format_options=CHARSET:UTF-8`;
...
/**
 * @function
 * @name create_CSV_GEOSERVER
 * @description
 * create csv data for geoserver layer
 * @param {Object} {data} layer data
 * @returns {csv data}
 */
export const create_CSV_GEOSERVER = () => {}
