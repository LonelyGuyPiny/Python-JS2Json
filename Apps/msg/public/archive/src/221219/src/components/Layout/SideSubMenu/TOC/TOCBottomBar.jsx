...
import { downloadKMZ, setAutoFocus, downloadGeoJSON, exportFeatures } from "../../../../redux/modules/toc";
...
class TOCBottomBar extends React.Component {
  ...
  createFeature(rowData) {
    try {
      const { geometryType, OIDName, featuresGeometry, tocLayer, crs, projection } = this.props;
      let geometry;
      ...
      return createFocusFeature_MIDDLEWARE(middlewareData);
    } catch (err) {
      // console.log("feature err", err);
    }
  }
  ...
  handleDownloadFile = async (type) => {
    ...
    if(type === 'kmz') {
      ...
      const res = await this.props.dispatch(exportFeatures(where, type));
      if (res && res.export) {
        this.setState({
          isExport: true,
        });
