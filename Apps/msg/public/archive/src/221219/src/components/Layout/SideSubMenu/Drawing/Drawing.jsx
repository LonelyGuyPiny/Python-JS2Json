...
import { formatArea, lengthFunction, circleMeasurement, measureLength, measureArea } from '../../../../utils/drawing';
import {
  setDwaingLayer,
  setDraingState,
  setDrawStyle,
...
class DrawingFunctionality extends Component {
  ...
  componentDidMount = () => {
    ...
    const drawingStoreData = store('drawingData');
    ...
    if ((data && (data.VectorSource.getFeatures().length > 0 || data.measureVectorSource.getFeatures().length > 0)) || (drawingStoreData && (drawingStoreData.vectorFeatures.length > 0 || drawingStoreData.measureVectorFeatures.length > 0))) {
      this.setState({
        showClearAll: true
      })
    }
    if ((data && data.measureVectorSource.getFeatures().length > 0) || (drawingStoreData && drawingStoreData.measureVectorFeatures.length > 0)) {
      this.setState({
  ...
  createStoreData = (storeFeature, type) => {
    try {
      ...
      // var measurementsToolTip = this.measurementsToolTip;
      // var measurementsSegmentsToolTip = this.measurementsSegmentsToolTip;
  ...
  onTabChange = (activeIndex) => {
    const { drawStyle, measureStyle } = this.props;
    const drawingStoreData = store('drawingData');
    if (activeIndex === 0) {
  ...
  deleteAllFeature = () => {
    const drawingdata = store('drawingData');
  ...
  handleAllClear = () => {
    ...
    if (drawingStoreData) {
      ...
      this.props.dispatch(setStoreData(drawingStoreData));
      if (drawingStoreData.vectorFeatures.length === 0 && drawingStoreData.measureVectorFeatures.length === 0) {
        this.setState({ showClearAll: false });
  ...
  handleGeoJsonDownload = () => {
    try {
      ...
      var measureGeom = [];
      let circleArr = [];
      drawingStoreData.vectorFeatures.forEach(vFeature => {
        let circleFeature = '[[';
