...
import { setStoreData } from '../../redux/modules/drawing';
...
class MenuSidebar extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (drawingData) {
      drawingData.measureVectorFeatures = [];
      drawingData.measureDrawIds = [];
      this.props.dispatch(setStoreData(drawingData));
    }
  ...
  createDrawingData = () => {
    const { projection, translation, storeData, language, selectedUnitType } = this.props;
    const drawingdata = Object.keys(storeData).length > 0 ? storeData : store('drawingData');
    if (!drawingdata) {
    ...
    if (drawingdata) {
      try {
        ...
        // const measureVectorLayer = new VectorLayer(measureVectorObject);
        // const compareMeasureVectorLayer = new VectorLayer(measureVectorObject);
  ...
  render() {
    const { translation, visibleLayers, compareBasemap, compareBasemapType, countLayers, compareMap, direction, language,
      updatedAt,
      selectedUnitType
    } = this.props;
