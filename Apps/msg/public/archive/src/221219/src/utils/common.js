...
const createWhereCondition = (filtersData, defaultWhere = null) => {
  ...
  try {
    ...
    Object.keys(filtersData).forEach((field, i) => {
      ...
      if (filtersData[field].type === 'esriFieldTypeString' || filtersData[field].type === 'string') {
        ...
        where += `${field} IN (${value})`;
      } else if (['esriFieldTypeInteger', 'esriFieldTypeSmallInteger', 'esriFieldTypeOID', 'esriFieldTypeDouble', 'esriFieldTypeSingle', 'int', 'number'].includes(filtersData[field].type)) {
