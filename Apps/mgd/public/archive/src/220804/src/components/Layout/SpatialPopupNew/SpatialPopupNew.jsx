...
class SpatialPopupNew extends Component {
  ...
  createVectorLayer = () => {
    ...
    this.compareMap.addLayer(this.vectorLayer);
    this.compareMap.addLayer(this.geometryVectorLayer);
  ...
  onMapSingleClick = (mapName) => async (evt) => {
    ...
    // this.setState((prevState) => ({ effectvisible: !prevState.effectvisible }))
    ...
    if (coordinate) {
      ...
      let showSpatial = true;
      Object.keys(selectedLayers).forEach(item => {
        if (selectedLayers[item].layerId === layerId) {
          if (selectedLayers[item].groupname) {
            showSpatial = false
          }
        }
      })
      this.setState({
        ...
        spatialData,
        currentPosition: 0,
        showSpetialIntersection: showSpatial
      });
  ...
  selectedLayer = layerid => {
    ...
    this.setState({ showSpetialIntersection: showSpatial });
  ...
  render() {
    ...
    const { isOpen, type, openClass, loading, filteredSpatialData, X, Y, currentPosition, zoomEnabled, optionLayerId, showSpetialIntersection, effectvisible } = this.state;
              ...
              >
                <Block id="popup-fixed" className="spatal-popup">
                  <PopUp
                    ...
                    direction={direction}
                    showIntersection={showSpetialIntersection}
                    // visible={effectvisible}
                  // blockStyle="block"
                  />
                </Block>
