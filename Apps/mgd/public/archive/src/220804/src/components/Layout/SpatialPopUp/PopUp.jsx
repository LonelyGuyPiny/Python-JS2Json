...
export default function PopUp({ loading, renderXYLinks, spatialData, X, Y, renderSpatialData, closePopUp, zoomInOut, currentPosition, previousClick, nextClick, zoomEnabled, translation, selectedLayer, options, drawingPopup, optionIndex, direction, showIntersection }) {
  ...
  if (showIntersection && menu.mainMenu.includes('layerRecords')) {
        ...
        <Block className="spatalPopupheader">
          ...
          {!drawingPopup && <Block className="card-spatal">
            <Block className="spatalPopupheaderTop">
              {/* <Block>
                <Svg className="spatial-tag" name="spatial-tag" onClick={handleClick} />
                <Svg className="spatial-fixed" name="spatial-fixed" onClick={handleClick} />
              </Block> */}
          ...
          {spatialData && spatialData.length > 0 &&
            <Block className="spatial-inner-sec">
              <Block className="spatalPopupheaderBottom">
                {/* <Block className="zoom-spatal">
                  <Svg onClick={() => setOpen(!open)} name="verticalmenulayer" className="cursor-pointer" />
                </Block> */}
                <Block className="titleText heading">
                  {/* <h3 className="font-weight-medium">{layerName}</h3> */}
              ...
              />
              {/* {open ? (
                <Block className="expand-zoom-spatal">
                  <Block className="spatial-checkbox">
                    <Checkbox
                      label={translation.autoZoom}
                      checked={zoomEnabled}
                      onChange={(e, { checked }) => zoomInOut(checked, spatialData[currentPosition])}
                    />
                  </Block>
                  <PopUpSpaitalIntersection
                    close={() => setOpen(false)}
                ...
                <Block>
                  {renderSpatialData(spatialData[currentPosition])}
                </Block>
              )} */}
              <Block className="spatial-checkbox">
                <Checkbox
