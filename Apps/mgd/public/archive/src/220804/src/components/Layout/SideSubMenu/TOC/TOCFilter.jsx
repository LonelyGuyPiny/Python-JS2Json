...
class DropDownMenu extends Component {
  ...
  handleSubmit = async (e) => {
    ...
    } else if (['esriFieldTypeOID', 'esriFieldTypeDouble', 'esriFieldTypeSingle', 'esriFieldTypeInteger', 'esriFieldTypeSmallInteger'].includes(type)) {
      if (!isRange) {
        filterData[name] = {
          field: name,
          type,
  ...
  renderFilterOpts = (type) => {
    if (this.domainValues || type === 'esriFieldTypeString') {
  ...
  renderStringFilter() {
    const { translation, fieldOptions, name, isLoading, direction } = this.props;
    ...
    let options = fieldOptions && fieldOptions[name] ? fieldOptions[name].map(v => ({
      key: v,
      value: v,
        ...
        <Block className='middleDropdownCol'>
          ...
          {isRange ? (
            ...
            <>
              ...
              <Block className="select-field mt-1">
                <label>{translation.selectValue}</label>
                <DatePicker
  ...
  render() {
    const { type, name, filtersData, direction } = this.props;
...
export default connect(
  state => ({
    ...
    isLoading: state.translation.isFilterOptionsLoading
  })
)(reduxForm({
  form: 'select-basemap',
