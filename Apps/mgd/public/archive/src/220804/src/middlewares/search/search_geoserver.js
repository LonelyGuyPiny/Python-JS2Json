...
export const createSearchSpatial_GEOSERVER = async (requestData) => {
  try {
    ...
    const findGeo = requestData.allLayers.find(al => al.groupname === requestData.combobox.layer && al.geometrytype);
    let geoLayerId = findGeo.id;
    let layerProjection = findGeo.projection;
