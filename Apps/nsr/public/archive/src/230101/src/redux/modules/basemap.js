...
const initialState = {
  ...
  selectedBasemap: store(`${basepath}-basemapSlug`) ? store(`${basepath}-basemapSlug`) : default_basemap,
  ...
  labelLayer: null,
  cLabelLayer: null,
  islabelLayer: store(`${basepath}-labelMap`) ? store(`${basepath}-labelMap`) : basemapList && basemapList.labelmap && basemapList.labelmap.defaultStatus ? true : false,
...
export const setLabelsVisible = (islabelLayer, onBasemapChange = false) => async (dispatch, getState, api) => {
  if (getState().basemap.isLabels) {
    ...
    store(`${basepath}-labelMap`, islabelLayer);
  }
}
...
export const setBasemap = (basemap, basemapType) => async (dispatch, getState, api) => {
  if (basemap) {
    store(`${basepath}-basemapSlug`, basemap);
    dispatch({
      type: SET_BASEMAP,
      payload: basemap
    });
...
export const createArealMaps = (map) => async (dispatch, getState, api) => {
  ...
  // console.log("selectedBasemap", selectedBasemap);
  let vectorLayers = [];
    ...
    vectorLayers = await Promise.all(basemapList.basemaps.map(async y => {
      try {
        ...
        const middlewareData = {
          ...
          url: y.url,
          slug: y.slug,
          defaultBasemap: selectedBasemap,
  ...
  // console.log("unavailableBaseMaps", unavailableBaseMaps);
  let slugMismatch = true;
  layers.map(l => {
    if (l.get('title') === selectedBasemap && !unavailableBaseMaps.includes(selectedBasemap)) {
      map.addLayer(l);
      dispatch({
        type: SET_BASEMAP_LAYER,
        payload: l
      ...
      slugMismatch = false;
    } else if (l.get('title') === 'osm_map' && unavailableBaseMaps.includes(selectedBasemap)) {
      l.setVisible(true)
      map.addLayer(l);
      dispatch({
        type: SET_BASEMAP_LAYER,
  ...
  if (slugMismatch) {
    layers.map(l => {
      if (l.get('title') === default_basemap) {
        l.setVisible(true)
        map.addLayer(l);
        dispatch({
          type: SET_BASEMAP_LAYER,
