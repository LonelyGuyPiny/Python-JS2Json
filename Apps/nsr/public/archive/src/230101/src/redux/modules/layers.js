...
const SET_LAYER_GEOMETRY = 'layers/SET_LAYER_GEOMETRY';
...
export default function reducer(state = initialState, action) {
  switch (action.type) {
    ...
    case SET_LAYER_GEOMETRY:
      return {
        ...state, layerGeometry: action.payload
      }
    default:
      return state;
  }
...
export const fetchLayers = () => async (dispatch, getState, api) => {
  ...
  try {
    ...
    results.map((r, apisIndex) => {
      ...
      if (r && r.isTimeOut) {
        if (r.layerapi.group) {
          layersapi.forEach(api => {
            if (api.group && api.group.includes(r.layerapi.group.split(':')[0])) {
              timeOutApis.push(api);
            }
          })
        } else {
  ...
  try {
    ...
    results.map((r, apisIndex) => {
      ...
      const middlewareData = {
        source: r.serverType,
        text: r.text,
        url: TimeoutLayerApis[apisIndex].url,
        virtualGroup: TimeoutLayerApis[apisIndex].virtualGroup,
        export: r.export,
        nameSpaceGroupArr,
        length: updateLayer.length
      }
...
export const showLayer = (layer, parentLayer, subLayer) => async (dispatch, getState, api) => {
  ...
  if (filterLayers[layerId]) {
    ...
    if (e) {
      if(groupname) {
        ...
        const layerDefs = await dispatch(getLayerDefination([layerId, ...visibleLayers.filter(v => v.includes(parentLayerId))], eParams));
        eParams.CQL_FILTER = layerDefs;
        ceParams.CQL_FILTER = layerDefs;
      ...
      if(groupname) {
        const layerDefs = await dispatch(getLayerDefination([layerId, ...visibleLayers.filter(v => v.includes(parentLayerId))], {LAYERS: groupname}))
        const [l, cl] = createGeoLayer(layer, parentLayerId, opacity, undefined, layerDefs);
        map.addLayer(l);
  ...
  if (groupname && !layerGeometry.includes(parentLayerId)) {
    const unauthorisedLayers = [];
    const featureLayers = allLayers.filter(al => al.parentlayerids.includes(parentLayerId) && !al.sublayerids);
    layerGeometry.push(parentLayerId);
    await Promise.all(featureLayers.map(async (l) => {
      const authToken = getTokenObj(l.url);
      const authTokenData = authToken.token ? `&authkey=${authToken.token}` : '';
      const apiData = {
    ...
    dispatch({
      type: SET_LAYER_GEOMETRY,
      payload: layerGeometry
    });
    dispatch({
      type: SET_UNAUTH_LAYER_IDS,
      payload: unauthorisedLayers
  ...
  try {
    ...
    if (e) {
      if (groupname) {
        ...
        const layerDefs = await dispatch(getLayerDefination([...oldLayers.filter(v => v !== layerId)], eParams));
        eParams.CQL_FILTER = layerDefs;
        e.getSource().updateParams(eParams);
        ...
        const layerDefs = await dispatch(getLayerDefination([...oldLayers.filter(v => v !== layerId)]));
        eParams.layerDefs = layerDefs;
        e.getSource().updateParams(eParams);
    ...
    // console.log("hide layer err", err);
  }
}
...
export const removeFromSelectedLayerCounts = (layer, parentLayer, subLayer) => async (dispatch, getState) => {
  try {
    ...
    // console.log("removeFromSelectedLayerCounts err", err);
  }
}
...
export const removeFromSelectedLayers = (layer, parentLayer, subLayer) => async (dispatch, getState) => {
  try {
    ...
    // console.log("removeFromSelectedLayers", removeFromSelectedLayers);
  }
}
  ...
  try {
    ...
    if(geoLayer) {
      if (e) {
        ...
        const layerDefs = await dispatch(getLayerDefination([...cls.map(c => c.layerid), ...visibleLayers.filter(v => v.includes(parentLayerId))], eParams));
        eParams.CQL_FILTER = layerDefs;
        e.getSource().updateParams(eParams);
        ...
        }, parentLayerId, opacity, names.reverse().join(','), layerDefs);
        handleLayerLoader(l.getSource(), dispatch, getState);
        map.addLayer(l);
        compareMap.addLayer(cl);
        dispatch(setMap(map));
    ...
    if (geoLayer && !layerGeometry.includes(parentLayerId)) {
      const unauthorisedLayers = [];
      const featureLayers = allLayers.filter(al => al.parentlayerids.includes(parentLayerId) && !al.sublayerids);
      layerGeometry.push(parentLayerId);
      await Promise.all(featureLayers.map(async (l) => {
    ...
    // console.log("selectAllLayersByParentId err", err);
  }
}
...
export const showSearchLayer = (selectedLayer) => async (dispatch, getState) => {
  try {
    ...
    if (!visibleLayers.includes(layerid)) {
      ...
      if (!sl) {
        selectedLayers[layerid] = {
          ...
          BoundingBox: selectedLayer.boundingbox,
    ...
    if (groupname && !layerGeometry.includes(groupname.split(':')[0])) {
      const nameSpaceGroup = groupname.split(':')[0];
      const unauthorisedLayers = [];
      const featureLayers = allLayers.filter(al => al.groupname && al.groupname.includes(nameSpaceGroup) && !al.sublayerids);
      layerGeometry.push(nameSpaceGroup);
      await Promise.all(featureLayers.map(async (l) => {
        const authToken = getTokenObj(l.url);
        const authTokenData = authToken.token ? `&authkey=${authToken.token}` : '';
        const apiData = {
    ...
    // console.log("search layer err", err);
  }
}
...
export const filterLayer = (layer) => async (dispatch, getState) => {
  ...
  if (e) {
    const eParams = e.getSource().getParams();
    if (groupname) {
      const layerDefs = await dispatch(getLayerDefination(visibleLayers.filter(v => v.includes(parentLayerId)), eParams));
      eParams.CQL_FILTER = layerDefs;
    } else {
...
/**
 * @function
 * @name setLayersGeometry
 * @description
 * set the layer geometry for geoserver layers
 * @returns {layers geometry}
 */

export const setLayersGeometry = () => async (dispatch, getState) => {
  try {
    let {
      allLayers,
      layerGeometry
    ...
    const unauthorisedLayers = [];
    const groupNameArr = [];
    const featureLayers = allLayers.filter(al => al.groupname !== undefined && !al.sublayerids);
    await Promise.all(featureLayers.map(async (l) => {
      if (layerGeometry.includes(l.groupname.split(':')[0])) {
        return;
      } else if (!layerGeometry.includes(l.groupname.split(':')[0])) {
        groupNameArr.push(l.groupname.split(':')[0]);
      }
    ...
    dispatch({
      type: SET_LAYER_GEOMETRY,
      payload: groupNameArr
    });
  } catch (err) {
    // console.log("search geometry err", err);
  }
}
