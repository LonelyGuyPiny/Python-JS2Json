  ...
  try {
    ...
    visibleLayers.forEach(ln => {
      ...
      if (lData) {
        ...
        dispatch(showSelectedLayerLegends(lData, lData.url, lData.id, lData.parentlayerids[0], lData.groupname));
        dispatch(fetchAllLayersDataByParentId(pIds[0]));
      }
    })
...
export const getQueryUrlForSpaital = (coordinates) => async (dispatch, getState, api) => {
  try {
    ...
    console.log("getQueryUrlForSpaital err", err);
    return false;
  }
}
...
export const fetchSearchQueryData = (coordinates, geometryType) => async (dispatch, getState) => {
  try {
    ...
    console.log("fetchSearchQueryData err", err)
  }
}
