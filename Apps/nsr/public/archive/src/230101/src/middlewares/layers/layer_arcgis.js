...
* @returns {export data}
*/
export const export_ARCGIS = async(data) => {
  switch(data.type) {
    case 'csv':
...
* @returns {csv data}
*/
export const export_CSV = (exportData) => {
  try {
    if (exportData) {
      return {
        export: true
...
export const export_KMZ = async(exportData) => {
  try {
    await downloadKMZFile(`${exportData.layer.url}/${exportData.layer.id}/query?where=1=1&outFields=*&f=kmz${getTokenForUrl(exportData.layer.url)}`, 'kmz');
    return true;
  } catch (err) {
    // console.log("err", err);
    return false;
...
export const export_GeoJSON = async(exportData) => {
  try {
    await downloadKMZFile(`${exportData.layer.url}/${exportData.layer.id}/query?where=1=1&outFields=*&f=GeoJSON${getTokenForUrl(exportData.layer.url)}`, 'json');
    return true;
  } catch(err) {
    // console.log("err", err);
    return false;
...
/**
 * @function
 * @name create_CSV_ARCGIS
 * @description
 * create csv data for arcgis layer
 * @param {Object} {data} layer data
 * @returns {csv data}
 */
export const create_CSV_ARCGIS = async (exportData) => {
  try {
    const data = {
      where: '1=1',
      ...
      outFields: '*',
      f: `json`,
      ...getTokenObj(exportData.layer.url)
    }
    ...
    const getResult = async (offset = 0) => {
      ...
      // console.log();
      const results = await superagent.post(`${exportData.layer.url}/${exportData.layer.id}/query`).send(data).set({
        'Content-Type': 'application/x-www-form-urlencoded',
      });
