  ...
  try {
    ...
    requestData.layerIds.forEach(layerId => {
      ...
      if (geometry && Object.keys(filtersData).length > 0) {
        ...
        where = createWhereCondition_ARCGIS(whereObj);
  ...
  try {
    ...
    Object.keys(filtersData).forEach((field, i) => {
      ...
      if (filtersData[field].type === 'esriFieldTypeString') {
        ...
        // if (filtersData[field].isDomainValues) {
        //   // for single value allowed
        //   // value = value.includes("'") ? value.replaceAll("'", "''") : value;
        //   // for multiple values allowed
        //   if (filtersData[field].condition === 'same') {
        //     if (value.length > 1) {
        ...
        //     }
        //   }
        //   if (Object.keys(filtersData).length > 1) {
        //     where = `(${where})`;
        //   }
        // } else {
          if (filtersData[field].condition === 'same') {
            ...
            where += `${field} NOT IN ('${value.join("','")}')`;
          }
          // if (Object.keys(filtersData).length > 1 && filtersData[field].isDomainValues) {
          //   where = `(${where})`;
          // }
        // }
        ...
        if (filtersData[field].isDomainValues) {
          ...
          if (value.length > 1) {
            value.forEach((val, ind) => {
              if (ind === 0) {
                domainWhere += `${field} = ${val} OR `;
              } else if (ind === value.length - 1) {
                domainWhere += `${field} = ${val}`;
              } else {
                domainWhere += `${field} = ${val} OR `;
              }
            })
          } else {
            domainWhere += `${field} = ${value[0]}`;
          }
          // console.log("where", where);
          // console.log("Object.keys(filtersData).length", Object.keys(filtersData).length);
          if (Object.keys(filtersData).length > 1) {
            where += `(${domainWhere})`;
          } else {
            where += domainWhere;
