...
class LayerMenuItem extends Component {
  ...
  render() {
    const {
      layer, visibleLayers, filterLayers, scale, isFilter, tocLayer, translation,direction,
      layersData
    } = this.props;
    ...
    return (
      <Block className={`list ${!isLayerInScale ? 'opc-4' : ''} ${isVisible ? 'active' : ''} ${activeClass ? 'layerSelect' : ''}`}>
        ...
        <span className="icons">
          {isVisible &&
            <React.Fragment>
              <span className="margin-iconslayer filter-hover" data-tooltip={translation.removefilter} data-position={`${direction === 'RTL' ? 'right' : 'left'} center`} >
                {((isFilter && tocLayer && tocLayer.layerId === layer.layerId) ||
                (layersData[layer.layerId] && layersData[layer.layerId].isFilter)) &&
                  <><Svg className="filter-icon" name='layerfilterselected' /><Svg className="close-icon" name="close-icon" onClick={() => this.setState({ isClearFilter: true })}/></>
                }
              </span>
