...
import { Block, Loading, Language,
  // PreLoader
} from '../../components';
import { MenuSidebar, Magnifier, MapHistory, ZoomButtons, SpatialPopupNew } from '../../components/Layout';
import { setMap, setCompareMap, setMapData } from '../../redux/modules/map';
...
class Dashboard extends Component {
  ...
  componentDidMount = async () => {
    ...
    const element = document.getElementById('preloader-image');
    setTimeout(() => {
      element.style.display = "none";
    }, 1000)
    this.habndleMapLoading();
    const basepath = sessionStorage.getItem('basepath');
    const lang = store(`${basepath}-lang`) || settings.defaultLanguage
    this.props.dispatch(changeLanguage(lang))
    map.on('moveend', this.handleMoveEvent);
    // element.style.display = "none";
  }
  ...
  handleMoveEvent = () => {
    ...
    this.props.dispatch(setMapData(data))
  }
  ...
  componentWillUnmount = () => {
    ...
    if (map) {
      ...
      map.un('moveend', this.handleMoveEvent);
    }
  }
  ...
  createMap = async (selfProps) => {
    const { dispatch, ViewData } = selfProps;
    ...
    const viewObj = {
      center: ViewData && ViewData.center ? [ViewData.center[0], ViewData.center[1]] : [mapData.centerX, mapData.centerY],
      // rotation: Math.PI / 6,
      rotation: ViewData && ViewData.rotation ? ViewData.rotation : 0,
      zoom: ViewData && ViewData.zoom ? ViewData.zoom : mapData.default_zoom,
